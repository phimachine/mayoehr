...
#         self.pos_ffn = PositionwiseFeedForward(d_model, d_inner, dropout=dropout)
#
#     def forward(self, dec_input,  enc_output, non_pad_mask, dec_enc_attn_mask=None):
#         dec_output, dec_slf_attn = self.slf_attn(
#             dec_input, dec_input, dec_input, mask=None)
#         dec_output *= non_pad_mask
#
#         # no strong spatial mapping between input and targets exist, but I retain this module.
#         dec_output, dec_enc_attn = self.enc_attn(
#             dec_output, enc_output, enc_output, mask=dec_enc_attn_mask)
#         dec_output *= non_pad_mask
#
#         # only one position x1 kernel pooling

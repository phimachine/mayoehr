    ...
    def new_sequence_reset(self):
        ...
        if self.reset:
            if self.palette:
                ...
                self.memory=Parameter(torch.Tensor(self.N, self.W).cuda(), requires_grad=False).uniform_(-stdv, stdv)
                # TODO is there a reason to reinitialize the parameter object? I don't think so. The graph is not carried over.

            self.last_usage_vector=Parameter(torch.Tensor(self.bs, self.N).cuda(), requires_grad=False).zero_()
            self.precedence_weighting=Parameter(torch.Tensor(self.bs, self.N).cuda(), requires_grad=False).zero_()
            self.temporal_memory_linkage=Parameter(torch.Tensor(self.bs, self.N, self.N).cuda(), requires_grad=False).zero_()
            self.last_read_weightings=Parameter(torch.Tensor(self.bs, self.N, self.R).cuda(), requires_grad=False).zero_()
            self.last_write_weighting=Parameter(torch.Tensor(self.bs, self.N).cuda(),requires_grad=False).zero_()

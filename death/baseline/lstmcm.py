...
class ChannelManager():
    ...
    def __init__(self, dataloader, batch_size, model=None, seqdims=(0, 1), staticdims=(2,), cuda=True):
    ...
    def push_states(self, states_tuple):
        ...
        for i, ch in enumerate(self.channels):
            if self.cuda:
                slice_states_tuple=tuple((state.index_select(1,Variable(torch.LongTensor([i]).cuda())) for state in states_tuple))
            else:
                slice_states_tuple=tuple((state.index_select(1,Variable(torch.LongTensor([i]))) for state in states_tuple))
    ...
    def get_states(self):
        ...
        rettup=tuple(torch.cat(st, dim=1) for st in batch_states)

...
from death.final.losses import DiscreteCrossEntropy

class TransformerMixedForwardSoftmax(nn.Module):
    ...
    def __init__(self, binary_criterion, real_criterion, prior=None, d_model=512, input_size=50000, d_inner=2048,
        ...
        self.position_enc = from_pretrained(get_sinusoid_encoding_table(max_position, d_model, padding_idx=0),
                                            freeze=True)

        self.layer_stack = nn.ModuleList([
            EncoderLayerAttn(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
    ...
    def forward(self, input, time_length, r=None):
        ...
        if r is None:
            ...
            embd = torch.matmul(input, (self.embedding + r))

        # -- Forward
        enc_output = embd + self.position_enc(src_pos)
...
class TransformerMixedAttnSoftmax(nn.Module):
    # no attention module here, because the input is not timewise
    # we will add time-wise attention to time-series model later.
    ...
    def __init__(self, binary_criterion, real_criterion, prior=None, d_model=512, input_size=50000, d_inner=2048,
        ...
        self.embedding = torch.nn.Parameter(torch.Tensor(input_size, d_model))
        # self.first_embedding=nn.Linear(vocab_size,d_model)
        self.position_enc=from_pretrained(get_sinusoid_encoding_table(max_position, d_model, padding_idx=0), freeze=True)
...
if __name__ == '__main__':
    ...
    model=TransformerMixedAttnSoftmax(binary_criterion=binary_criterion,real_criterion=real_criterion,
                               input_size=7298, output_size=435, prior=None).cuda()
    all_loss, lml, lat, lem, lvat, toe_loss, output=model.one_pass(input, time_length, target, loss_type)
    print(all_loss, lml, lat, lem, lvat, output)
    model=TransformerMixedForwardSoftmax(binary_criterion=binary_criterion,real_criterion=real_criterion,
                               input_size=7298, output_size=435, prior=None).cuda()
    all_loss, lml, lat, lem, lvat, toe_loss, output=model.one_pass(input, time_length, target, loss_type)
    print(all_loss, lml, lat, lem, lvat, output)
